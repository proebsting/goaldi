...
to which the method applies according to this table:
====
*x* {nbsp} any value +
*t* {nbsp} type value +
*f* {nbsp} file value +
...
S.insert(x[]) -- add members [DEPRECATED]::
S.insert(x...) [DEPRECATED] adds all its arguments to set S. It returns
the set S. The preferred form is now S.put(x...) or S @: x.
...
open(name,flags) -- open a file [DEPRECATED]::
