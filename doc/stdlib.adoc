...
+
With one argument: r is a color name, a grayscale value in (0, 1), or a
hexadecimal specification (#k, #kk, #rgb, #rgba, #rrggbb, #rrggbbaa).
+
...
P.color(r,g,b,a) sets the drawing color for painter P. The Painter value
...
Rename renames (moves) a file. OS-specific restrictions might apply. If
...
P.turn(d) adjusts the current orientation by d degrees. The Painter
value is returned to allow chaining.
